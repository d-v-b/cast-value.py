"""Test that CastValueRust fails clearly when cast-value-rs is not installed.
...
def test_rust_codec_importable_without_backend() -> None:
    """Test that CastValueRust can be imported and constructed without cast-value-rs."""
    codec = CastValueRust(data_type="uint8")
...
def test_rust_codec_encode_raises_without_backend() -> None:
    """Test that encoding with CastValueRust raises ModuleNotFoundError without cast-value-rs."""
    codec = CastValueRust(data_type="uint8")
...
def test_rust_codec_decode_raises_without_backend() -> None:
    """Test that decoding with CastValueRust raises ModuleNotFoundError without cast-value-rs."""
    codec = CastValueRust(data_type="uint8")
